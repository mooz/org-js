...
ConverterHTML.prototype = {
  ...
  prependCheckBoxToListItem: function (listItemText) {
    if (/^\s*\[(X| )\]([\s\S]*)/.exec(listItemText)) {
      // Embed checkbox
      var checkboxIndicator = RegExp.$1;
      var listItemContent = RegExp.$2;
      ...
      return this.inlineTag("input", null, attributes) + listItemContent;
    } else {
      return listItemText;
    }
  },
  ...
  convertListItem: function (node, childText, auxData) {
    if (this.exportOptions.suppressCheckboxHandling) {
      ...
      return this.tag("li", this.prependCheckBoxToListItem(childText), null, auxData);
    }
  },
