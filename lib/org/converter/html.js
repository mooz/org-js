...
ConverterHTML.prototype = {
  ...
  convertHeader: function (node, childText, auxData,
    ...
    if (taskStatus) {
      var taskHeading = taskStatus.heading,
          taskColor = taskStatus.color,
          restOfHeading = childText.substring(taskHeading.length + 1);

      var taskClasses = "task-status "
            + "task-" + taskHeading.toLowerCase();
      var colorClass = "task-color-" + taskColor.toLowerCase();

      childText = this.inlineTag("span", taskHeading, {
        "class": taskClasses + " " + colorClass
      }) + restOfHeading;

      headerAttributes["class"] = taskClasses;
    }
