...
Parser.prototype = {
  ...
  parseDocument: function () {
    ...
    while (this.lexer.hasNext()) {
      ...
      if (element) this.document.appendNode(element);
    }
  ...
  interpretOptionDirective: function (optionDirectiveNode) {
    optionDirectiveNode.directiveArguments.forEach(function (pairString) {
      ...
      this.documentOptions[pair[0]] = this.convertLispyValue(pair[1]);
    }, this);
  },
...
InlineParser.prototype = {
  ...
  validURLPattern: /\b(?:https?:\/\/|www\d{0,3}[.]|[a-z0-9.\-]+[.][a-z]{2,4}\/)(?:[^\s()<>]+|\(([^\s()<>]+|(\([^\s()<>]+\)))*\))+(?:\(([^\s()<>]+|(\([^\s()<>]+\)))*\)|[^\s`!()\[\]{};:'".,<>?«»“”‘’])/i,
  imageExtensions : [
