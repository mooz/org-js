...
function InlineParser() {
  ...
  this.timestampPattern =/([\[<])(\d{4}-\d{2}-\d{2})(.+?)?([\]>])/g;
}

InlineParser.prototype = {
  parseTimestamp: function(text) {
    var timestampPattern = this.timestampPattern;
    ...
    while ((match = timestampPattern.exec(text))) {
      ...
      {
        // parse links
        var matchBegin = timestampPattern.lastIndex - whole.length;
        var beforeContent = text.substring(previousLast, matchBegin + pre.length);
        savedLastIndex = timestampPattern.lastIndex;
        result.push(Node.createTimestamp(whole.substring(1, whole.length-1)));
        timestampPattern.lastIndex = savedLastIndex;
      }

      previousLast = timestampPattern.lastIndex;
    }

    if (timestampPattern.lastIndex === 0 ||
        timestampPattern.lastIndex !== text.length - 1)
  ...
  parseLink: function (text) {
    ...
    if (linkPattern.lastIndex === 0 ||
      ...
      result.push(this.parseTimestamp(text.substring(previousLast)));
