...
InlineParser.prototype = {
  ...
  _getMatchesForTextByPattern: function(text, pattern, onMatch, nextParser) {
    var myPattern = pattern;
    myPattern.lastIndex = 0;

    if(!nextParser) {
      nextParser = function(s) {
        return Node.createText(null, { value: s });
      }
    }
    ...
    var result = [],
        match,
        lastMatchEndIndex = 0;
      while(match = myPattern.exec(text)) {
        if(match.index > lastMatchEndIndex) {
          var beforeContent = text.substring(lastMatchEndIndex, match.index);
          result.push(nextParser.call(this, beforeContent));
        }
        var maybeNode = onMatch(match);
        if(maybeNode) {
          result.push(maybeNode);
        } else {
          result.push(nextParser(match[0]))
        }
        lastMatchEndIndex = match.index + match[0].length;
      }

      if (lastMatchEndIndex < text.length)
        result.push(nextParser.call(this, text.substring(lastMatchEndIndex)));
  ...
  // TODO: support timestamp ranges i.e. <TS1>--<TS2>
  // <2017-05-28 Sun> # active
  // [2017-05-28 Sun] # inactive
  // [2017-05-28 Sun 13:05] # with time
  parseTimestamp: function(text) {
    return this._getMatchesForTextByPattern(
      text, this.timestampPattern,
      function(match) {
        var whole  = match[0],
            pre    = match[1],
            date   = match[2],
            ...
            time_  = match[4],
            post   = match[5];
        var datetimeString = date + (time_ ? " "+time_:"");

        var timestamp = Node.createTimestamp(whole);
        timestamp.isActive = null;
        timestamp.date = Date.parse(datetimeString);

        switch(pre+post) {
          case "<>": timestamp.isActive = true; break;
          case "[]": timestamp.isActive = false; break;
        }
        if(timestamp.isActive !== null) {
          return timestamp;
        }
      }
    )
  ...
  parseMacro: function(text) {
    return this._getMatchesForTextByPattern(
      text, this.macroPattern,
      function(match) {
        var whole  = match[0];
        var name   = match[1];
        var argstr = match[2];
        var macro = Node.createMacro([]);
        macro.name = name;
        macro.args = argstr.split(",").map(function(s) {
  ...
  parseLink: function (text) {
    ...
    if (linkPattern.lastIndex === 0 ||
      ...
      result.push(this.parseMacro(text.substring(previousLast)));
